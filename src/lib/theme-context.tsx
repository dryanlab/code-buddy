...
export function ThemeProvider({ children }: { children: ReactNode }) {
  const [themeId, setThemeIdState] = useState("cyber");
  const [hasChosenTheme, setHasChosenTheme] = useState(true);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    const saved = localStorage.getItem("code-buddy-theme");
    const chosen = localStorage.getItem("code-buddy-theme-chosen");
  ...
  const setThemeId = useCallback((id: string) => {
    setThemeIdState(id);
    localStorage.setItem("code-buddy-theme", id);
  }, []);

  const markThemeChosen = useCallback(() => {
    setHasChosenTheme(true);
    localStorage.setItem("code-buddy-theme-chosen", "true");
  }, []);
